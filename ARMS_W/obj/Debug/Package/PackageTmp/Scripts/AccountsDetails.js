...
function LoadDeliveryAddresses(obj, code, address) {
    var str_rows = "";
    var rowid = $('#' + obj + ' tr').length - 1;
    rowid++;
    str_rows = str_rows + '<tr RowId="' + rowid + '">';
    str_rows = str_rows + '<td style="display:none;"><input type="hidden" value="' + code + '" /></td>';
    str_rows = str_rows + '<td><input type="text" value="' + address + '" ' + (changeStatus != "1" ? "readonly=\"readonly\" class=\"readonly_fields\"" : "") + ' style="width:99%;" /></td>';
    str_rows = str_rows + '<td ' + (changeStatus != "1" ? "style=\"display:none;\"" : "") + '><a href="javascript:DelCurrRow(\'' + obj + '\',' + rowid + ');"><img src="' + baseUrl + 'Images/delete.png" style="border:0;" /></a></td>';
    str_rows = str_rows + '</tr>';
    $('#' + obj + ' tr:last').before(str_rows);
}

//function add delivery address
function AddDeliveryAddress() {
    var hasDuplicate = false;
    $('#tblDeliveryAddress tbody tr td:nth-child(2) input').each(function (index, item) {
        if ($(item).attr('id') != 'txt_delivery_add') {
            if ($(item).attr('value') == $('#txt_delivery_add').attr('value')) {
    ...
    if (!hasDuplicate) {
        var last_id = $('#tblDeliveryAddress tr:last').prev().attr('RowID');
        var new_id = 0;
        if (last_id != undefined) {
            new_id = parseInt(last_id) + 1;
        }
        $('#tblDeliveryAddress tr:last').before("<tr RowID=\"" + new_id + "\">" +
        "<td style=\"display:none;\"><input type=\"hidden\" value=\"generateNew\" /></td>" +
        "<td><input type=\"text\" value=\"" + $('#txt_delivery_add').attr('value') + "\" style=\"width:99%;\" /></td>" +
        "<td><a href=\"javascript:DelCurrRow('tblDeliveryAddress', " + new_id + ");\"><img src=\"" + baseUrl + "Images/delete.png\" style=\"border:0;\" /></a></td>" +
        "</tr>");
    }
}
...
function DisableEditing(val_business_type) {
    ...
    //
    $('#tblDeliveryAddress a').attr('disabled', 'disabled');
...
function CheckRequiredFields() {
    ...
    return checkForDuplicateAddress();
...
function checkForDuplicateAddress() {
    ...
    $('#tblDeliveryAddress tbody tr td:nth-child(2) input').each(function (index, item) {
        var thisAddress = $(this).attr('value');
        if ($.inArray(thisAddress, allAddress) === -1) {
...
function Savedata() {
    ...
    //delivery addresses
    var list_of_delivery_addresses = new Array();
    $('#tblDeliveryAddress tbody tr td:nth-child(2) input').each(function (index, item) {
        if ($(item).attr('value').replace(/\s/g, '') != "") {
            list_of_delivery_addresses.push($(item).attr('value'));
        }
    });
    ...
    var params = {
        ...
        list_of_delivery_address: list_of_delivery_addresses //added line by RRR 2/21/2017
    };
