...
<asp:Content ID="Content1" ContentPlaceHolderID="MainContent" runat="server">
    <% 
        ...
        string cardcode = "", cardname = "", datecreated = "", doccreator = "", status = "", pageno = "1", area = "", forSapUpdate = "";
        ...
        if (Request.QueryString["cardcode"] != "" && Request.QueryString["cardcode"] != null)
        {
            ...
            ActMsg.Add("Searched " + cardcode + " keyword on Customer Code on the List of Customer");
        }

        if (Request.QueryString["cardname"] != "" && Request.QueryString["cardname"] != null)
        {
            cardname = Request.QueryString["cardname"].ToString();
            ActMsg.Add("Searched " + cardname + " keyword on Customer Name on the List of Customer");
        }
        ...
        if (Request.QueryString["datecreated"] != "" && Request.QueryString["datecreated"] != null)
        {
            ...
            ActMsg.Add("Searched " + datecreated + " keyword on Date Created on the List of Customer");
        }

        if (Request.QueryString["doccreator"] != "" && Request.QueryString["doccreator"] != null)
        {
            doccreator = Request.QueryString["doccreator"].ToString();
            ActMsg.Add("Searched " + doccreator + " keyword on Encoder on the List of Customer");
        }

        if (Request.QueryString["area"] != "" && Request.QueryString["area"] != null)
        {
            area = Request.QueryString["area"].ToString();
            ActMsg.Add("Searched " + area + " keyword on Area on the List of Customer");
        }

        if (Request.QueryString["status"] != null)
        {
            status = Request.QueryString["status"].ToString();
            ActMsg.Add("Filtered the '" + AppHelper.GetDocStateMsg(status) + "' status on the List of Customer");
        ...
        if (Request.QueryString["pageno"] != "" && Request.QueryString["pageno"] != null)
        {
            ...
            ActMsg.Add("Moved to the page no." + pageno + " on the List of Customer");
        }
        
        if (Request.QueryString["forSapUpdate"] != "" && Request.QueryString["forSapUpdate"] != null)
        {
            forSapUpdate = Request.QueryString["forSapUpdate"].ToString();
        }
    ...
    <script src="<%=ResolveUrl("~/") %>Scripts/list_accounts.js" language="javascript" type="text/javascript"></script>
    <script type="text/javascript" language="javascript">    
        var baseUrl = "<%= ResolveUrl("~/") %>";
    </script>

    <div class="bl_box" style=" height:100%; " >
		...
		<table id="tbl_lst_accounts" width="100%" cellpadding="3" cellspacing="0" border="0" >
            ...
            <% 
                ...
                mtable = SqlDbHelper.getDataDT(SqlQueryHelper.LifOfAccounts(cardcode, cardname, datecreated, doccreator, status, area, forSapUpdate));
                bool CanViewTheDocument = false;
                ...
                {
                    String Status = AppHelper.GetAccDocStatusMessage(LDocs.DocList[i][3].ToString(), LDocs.DocList[i][4].ToString(), LDocs.DocList[i][0].ToString(), LDocs.DocList[i][6].ToString(), LDocs.DocList[i][7].ToString());
                    Response.Write("<tr>");
                    ...
                    Response.Write("<td align=\"left\">" + (Status=="For Channel Manager Approval" && (LDocs.DocList[i][12].ToString()).Contains("TRADE") ? "For RSM Approval" : Status) + "</td>");
                    Response.Write("</tr>");
                }
                ...
                other_options = ResolveUrl("~/") + "Document/Accounts";
