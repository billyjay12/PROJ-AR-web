...
<asp:Content ID="Content1" ContentPlaceHolderID="MainContent" runat="server">
    <% 
        ...
        string CurrDocMessage = AppHelper.GetAccDocStatusMessage(acct_dtls.curr_doc_DocStatus, acct_dtls.curr_doc_DocChangesStatus, DocCCaNum, "", acct_dtls.proposedChangesCA.is_sent_back);
    ...
    <script type="text/javascript" language="javascript">
        ...
        var changeStatus = '<%:acct_dtls.curr_doc_DocChangesStatus %>';
...
//          $("#tbl_specified_events img").show();
        ...
        function LoadData() {
            ...
            g_proposed_address = "<% Response.Write(acct_dtls.proposedDelAddresses.Count); %>";
            ...
            // delivery addresses
            if(g_proposed_address.toString()=="0"){
                <% foreach(ARMS_W.SkelClass.Document.AccountDetails.delivery_addresses row in acct_dtls.delAddresses) { %>
                    <% Response.Write("LoadDeliveryAddresses('tblDeliveryAddress', '', '"+row.ch_address+"');"); %>
                <% } %>
            }
            else{
                <% foreach(ARMS_W.SkelClass.Document.AccountDetails.delivery_addresses row in acct_dtls.proposedDelAddresses) { %>
                    <% Response.Write("LoadDeliveryAddresses('tblDeliveryAddress', '', '"+row.ch_address+"');"); %>
                <% } %>
            }
	...
	<div id="tabs-1">
        ...
        <table width="100%" cellpadding="1" cellspacing="0" border="0">
			...
			<tr>
				...
				<td colspan="5">
                    <table id="tblDeliveryAddress" style="width:100%;" cellpadding="0" cellspacing="0">
                        <tbody>
                            <tr class="last_row">     
                                <td style="display:none;"><input type="hidden" /></td>                           
                                <td><input type="text" id="txt_delivery_add" style="width:99%;" maxlength="100" /></td>
                                <td style="width:10px;"><a href="javascript:AddDeliveryAddress();"><img src="<%:ResolveUrl("~/") %>Images/add.png" /></a></td>
                            </tr>
                        </tbody>
                    </table>					
				</td>
