...
<asp:Content ID="Content1" ContentPlaceHolderID="MainContent" runat="server">
    ...
    <% 
        ...
        if (Request.QueryString["cardcode"] != "" && Request.QueryString["cardcode"] != null)
        {
            ...
            ActMsg.Add("Searched " + cardcode + " keyword on Customer Code on the List of Customer Change Approval");
        }

        if (Request.QueryString["cardname"] != "" && Request.QueryString["cardname"] != null)
        {
            cardname = Request.QueryString["cardname"].ToString();
            ActMsg.Add("Searched " + cardname + " keyword on Customer Name on the List of Customer Change Approval");
        }
        ...
        if (Request.QueryString["datecreated"] != "" && Request.QueryString["datecreated"] != null)
        {
            ...
            ActMsg.Add("Searched " + datecreated + " keyword on Date Created on the List of Customer Change Approval");
        }

        if (Request.QueryString["doccreator"] != "" && Request.QueryString["doccreator"] != null)
        {
            doccreator = Request.QueryString["doccreator"].ToString();
            ActMsg.Add("Searched " + doccreator + " keyword on Encoder on the List of Customer Change Approval");
        }

        if (Request.QueryString["area"] != "" && Request.QueryString["area"] != null)
        {
            area = Request.QueryString["area"].ToString();
            ActMsg.Add("Searched " + area + " keyword on Area on the List of Customer Change Approval");
        }

        if (Request.QueryString["status"] != null)
        {
            status = Request.QueryString["status"].ToString();
            ActMsg.Add("Filtered the '" + AppHelper.GetDocStateMsg(status) + "' status on the List of Customer Change Approval");
        ...
        if (Request.QueryString["pageno"] != "" && Request.QueryString["pageno"] != null)
        {
            ...
            ActMsg.Add("Moved to the page no." + pageno + " on the List of Customer Change Approval");
        }
    ...
    <script src="<%=ResolveUrl("~/") %>Scripts/AccountsWChanges.js" type="text/javascript"></script>
    ...
    <div class="simple_box" style="padding:0;" >
		<table id="tbl_lst_accounts" width="100%" cellpadding="2" cellspacing="0" border="0" >
           ...
           <% 
                ...
                mtable = SqlDbHelper.getDataDT(SqlQueryHelper.LifOfAccountsWChanges(cardcode, cardname, datecreated, doccreator, status, area));
                ...
                for (Int32 i = StartingRecordNumber; i < EndingRecordNumber && i < TotalDocs; i++)
                {
                   ...
                   // Response.Write("<td align=\"left\">" + (Status == "For Channel Manager Approval" && (LDocs.DocList[i][12].ToString()).Contains("TRADE") ? "For RSM Approval" : Status) + "</td>");
                    Response.Write("<td align=\"left\">" + Status + "</td>");
                    Response.Write("</tr>");
                }
                ...
                other_options = ResolveUrl("~/") + "Document/AccountsWChanges";
    ...
    <script type="text/javascript" language="javascript">
        $(function () {
            // alert($("#tbl_lst_accounts tr:nth-child(1)").find("td:nth-child(1)").width());
            /*
